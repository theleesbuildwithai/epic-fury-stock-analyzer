...
def build_version():
    return {
        "commit_marker": "feat-v119-stb-full-universe-scan-yfinance-download",
        "date": "2026-06-17",
        "fixes_in_build": [
            ...
            "v68_truth_engine_sp500_close_gspc_spy_10s_thread_wrappers",
            "v68_truth_engine_cumulative_return_gspc_spy_spx_10s_threads",
...
@app.post("/api/admin/clear-picks-cache")
def admin_clear_picks_cache(request: Request):
    """Clear the in-memory quant picks cache so the next call to
    generate_quant_picks() runs a full fresh scan with live L1 price
    correction. Use when picks show stale/corrupt prices (e.g. ARM=$540
    from yfinance auto_adjust bug) that survived the 5-min TTL."""
    check_rate_limit(request.client.host)
    try:
        from analysis.quant_engine import _quant_cache
        before_keys = list(_quant_cache.keys())
        _quant_cache.clear()
        logger.warning(f"ADMIN: cleared quant_cache keys={before_keys}")
        return {"ok": True, "cleared_keys": before_keys, "message": "Next generate_quant_picks() will run fresh scan with L1 price correction"}
    except Exception as e:
        return {"ok": False, "reason": str(e)[:300]}
...
@app.post("/api/admin/force-trade-now-v6")
def admin_force_trade_now_v6(request: Request):
    """v6: fires after BULL gate fix (52% not 60%), short score abs() fix,
    and ceasefire threshold relaxation. Direction enforcement inherited from v5.
    """
    import os as _os_v6f
    _flag_v6f = _os_v6f.path.join(_os_v6f.path.dirname(__file__), ".force_trade_now_v6_done")
    if _os_v6f.path.exists(_flag_v6f):
        return {"ok": False, "reason": "already_used", "message": "v6 already used this deploy"}
    check_rate_limit(request.client.host)
    try:
        from predictions.paper_trader import execute_trades_from_signals
        from analysis.quant_engine import generate_quant_picks
        ...
        # Dedup
        _v6_long_syms = {p.get("symbol") for p in conf_longs}
        conf_shorts = [p for p in conf_shorts if p.get("symbol") not in _v6_long_syms]
        ...
        logger.warning(f"FORCE-TRADE-NOW-v6: {original_long}+{original_short} picks → conf={len(conf_longs)}+{len(conf_shorts)} → opened={opened_count}")
        return {
            "ok": True,
            "endpoint": "/api/admin/force-trade-now-v6",
            "original_picks": {"longs": original_long, "shorts": original_short},
            "conf_filtered": {"longs": len(conf_longs), "shorts": len(conf_shorts), "min_conf": MIN_CONF},
            "opened": opened_count,
            "closed": closed_count,
            "skipped": len(skipped),
            "opened_tickers": [o.get("symbol") for o in result.get("opened", [])][:30],
            "skipped_reasons": [s.get("reason", "")[:200] for s in skipped][:10],
            "portfolio_after": result.get("portfolio_after", {}),
            "endpoint_status": "DISABLED — self-locked after this run",
        }
    except Exception as e:
        logger.error(f"Force-trade-now-v6 error: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@app.post("/api/admin/force-trade-now-v7")
def admin_force_trade_now_v7(request: Request):
    """v7: full diagnostics + trade fire after audit clear + v94 fixes.
    Captures ALL skip reasons. Direction/sanity inherited from v5/v6.
    """
    import os as _os_v7f
    _flag_v7f = _os_v7f.path.join(_os_v7f.path.dirname(__file__), ".force_trade_now_v7_done")
    if _os_v7f.path.exists(_flag_v7f):
        return {"ok": False, "reason": "already_used", "message": "v7 already used this deploy"}
    check_rate_limit(request.client.host)
    try:
        from predictions.paper_trader import execute_trades_from_signals
        from analysis.quant_engine import generate_quant_picks
        ...
        _v7_long_syms = {p.get("symbol") for p in conf_longs}
        conf_shorts = [p for p in conf_shorts if p.get("symbol") not in _v7_long_syms]
        ...
        pick_snapshot = [
            {"symbol": _p.get("symbol"), "direction": _p.get("direction"),
             "confidence": _p.get("confidence"), "composite_score": _p.get("composite_score"),
             "price": _p.get("price")}
            for _p in conf_longs + conf_shorts
        ]
        ...
        logger.warning(f"FORCE-TRADE-NOW-v7: {original_long}+{original_short} → conf={len(conf_longs)}+{len(conf_shorts)} → opened={opened_count} skipped={len(skipped)}")
        return {
            "ok": True,
            "endpoint": "/api/admin/force-trade-now-v7",
            "original_picks": {"longs": original_long, "shorts": original_short},
            "conf_filtered": {"longs": len(conf_longs), "shorts": len(conf_shorts), "min_conf": MIN_CONF},
        ...
        logger.error(f"Force-trade-now-v7 error: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@app.post("/api/admin/force-trade-now-v9")
def admin_force_trade_now_v9(request: Request):
    """v9: clears stale quant_cache FIRST so generate_quant_picks() runs
    fresh L1 price correction (multi_source_quote_batch). Stale cache was
    serving ARM=$540, NXPI=$404, AMAT=$567 — all corrupt yfinance prices.
    Fresh regen corrects to live prices before picks reach execution.
    """
    import os as _os_v9f
    _flag_v9f = _os_v9f.path.join(_os_v9f.path.dirname(__file__), ".force_trade_now_v9_done")
    if _os_v9f.path.exists(_flag_v9f):
        return {"ok": False, "reason": "already_used", "message": "v9 already used this deploy"}
    check_rate_limit(request.client.host)
    try:
        # STEP 1: Clear stale picks cache so fresh L1-corrected prices are used
        try:
            from analysis.quant_engine import _quant_cache
            _quant_cache["quant_picks"] = {}
            _quant_cache["timestamp"] = 0
            logger.warning("FORCE-TRADE-v9: cleared _quant_cache — will regen with fresh prices")
        except Exception as _cc_e:
            logger.error(f"FORCE-TRADE-v9: cache clear failed: {_cc_e}")

        from predictions.paper_trader import execute_trades_from_signals
        from analysis.quant_engine import generate_quant_picks

        # STEP 2: Generate fresh picks (L1 correction runs on live prices)
        picks = generate_quant_picks()
        if not isinstance(picks, dict):
            return {"ok": False, "reason": "picks_gen_failed"}
        original_long = len(picks.get("long_picks", []))
        ...
        _v9_long_syms = {p.get("symbol") for p in conf_longs}
        conf_shorts = [p for p in conf_shorts if p.get("symbol") not in _v9_long_syms]
        ...
        logger.warning(f"FORCE-TRADE-NOW-v9: {original_long}+{original_short} → conf={len(conf_longs)}+{len(conf_shorts)} → opened={opened_count} skipped={len(skipped)}")
        return {
            "ok": True,
            "endpoint": "/api/admin/force-trade-now-v9",
            "cache_cleared": True,
            "original_picks": {"longs": original_long, "shorts": original_short},
            "conf_filtered": {"longs": len(conf_longs), "shorts": len(conf_shorts), "min_conf": MIN_CONF},
            "picks_sent": pick_snapshot,
...
@app.post("/api/admin/force-trade-now-v8")
def admin_force_trade_now_v8(request: Request):
    """v8: fires after options NAV audit fix. Same logic as v7."""
    import os as _os_v8f
    _flag_v8f = _os_v8f.path.join(_os_v8f.path.dirname(__file__), ".force_trade_now_v8_done")
    if _os_v8f.path.exists(_flag_v8f):
        return {"ok": False, "reason": "already_used", "message": "v8 already used this deploy"}
    check_rate_limit(request.client.host)
    try:
        from predictions.paper_trader import execute_trades_from_signals
        from analysis.quant_engine import generate_quant_picks
        ...
        _v8_long_syms = {p.get("symbol") for p in conf_longs}
        conf_shorts = [p for p in conf_shorts if p.get("symbol") not in _v8_long_syms]
        ...
        logger.warning(f"FORCE-TRADE-NOW-v8: {original_long}+{original_short} → conf={len(conf_longs)}+{len(conf_shorts)} → opened={opened_count} skipped={len(skipped)}")
        return {
            "ok": True,
            "endpoint": "/api/admin/force-trade-now-v8",
            "original_picks": {"longs": original_long, "shorts": original_short},
            "conf_filtered": {"longs": len(conf_longs), "shorts": len(conf_shorts), "min_conf": MIN_CONF},
            "picks_sent": pick_snapshot,
            "opened": opened_count,
        ...
        logger.error(f"Force-trade-now-v8 error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
