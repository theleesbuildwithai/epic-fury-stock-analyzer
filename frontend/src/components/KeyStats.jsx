export default function KeyStats({ info, latest, supportResistance }) {
  ...
  const stats = [
    ...
    { label: 'Change', value: `${isUp ? '+' : ''}$${priceChange.toFixed(2)} (${isUp ? '+' : ''}${priceChangePct}%)`, color: isUp ? 'text-green-400' : 'text-red-400' },
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
        {stats.map((stat) => (
          <div key={stat.label} className="bg-slate-900/50 rounded-lg p-3">
            ...
            <p className={`text-sm font-medium ${stat.color || 'text-slate-200'}`}>{stat.value}</p>
          </div>
        ))}
      </div>
      ...
      {supportResistance && (
        <div className="mt-4 grid grid-cols-2 gap-4">
          <div className="bg-green-900/20 rounded-lg p-3 border border-green-800/30">
            <p className="text-green-400 text-xs font-medium mb-1">Support Levels (Floor)</p>
            {supportResistance.support.length > 0
              ? supportResistance.support.map((s, i) => (
                  <span key={i} className="text-green-300 text-sm mr-2">${s.toFixed(2)}</span>
                ))
              : <span className="text-slate-500 text-sm">No clear levels</span>
            }
          </div>
          <div className="bg-red-900/20 rounded-lg p-3 border border-red-800/30">
            <p className="text-red-400 text-xs font-medium mb-1">Resistance Levels (Ceiling)</p>
            {supportResistance.resistance.length > 0
              ? supportResistance.resistance.map((r, i) => (
                  <span key={i} className="text-red-300 text-sm mr-2">${r.toFixed(2)}</span>
                ))
              : <span className="text-slate-500 text-sm">No clear levels</span>
            }
          </div>
